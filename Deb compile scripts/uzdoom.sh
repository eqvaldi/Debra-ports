...
mkdir -p "$DIR_NAME/usr/lib/uzdoom"
mkdir -p "$DIR_NAME/usr/share/applications"

# 5. Copy the compiled binaries, script libraries, and sound assets into staging
...
if [ -f "$SOURCE_DIR/build/uzdoom" ]; then
    ...
    # Stash mandatory system script package frameworks (.pk3 targets)
    cp "$SOURCE_DIR/build/uzdoom.pk3" "$DIR_NAME/usr/lib/uzdoom/"
    cp "$SOURCE_DIR/build/brightmaps.pk3" "$DIR_NAME/usr/lib/uzdoom/"
    cp "$SOURCE_DIR/build/lights.pk3" "$DIR_NAME/usr/lib/uzdoom/"
    cp "$SOURCE_DIR/build/game_widescreen_gfx.pk3" "$DIR_NAME/usr/lib/uzdoom/"
    cp "$SOURCE_DIR/build/game_support.pk3" "$DIR_NAME/usr/lib/uzdoom/"
    
    # Mirror sound fonts asset directories completely
    if [ -d "$SOURCE_DIR/build/soundfonts" ]; then
        cp -r "$SOURCE_DIR/build/soundfonts" "$DIR_NAME/usr/lib/uzdoom/"
    fi
else
